...
|---|---|---|
| Architecture (IV) | Between-subjects | Centralized, Federated, Intent-Driven, MCP Mediator, Multi-Agent |
| MCP Count (IV) | Within-subjects | 10, 20, 50, 100 |
| Model (IV) | Between-subjects | Gemini 2.0 Flash, GPT-5.3-Codex, Ministral-3B, Llama 3.3 70B |
| Workflow (IV) | Within-subjects | Code Investigation, Incident Investigation, Full System Audit |

### Controls

- **Same MCPs** — All architectures receive identical tool schemas (generated by `common_tools/factory.py`)
- **Same workflows** — Identical task prompts across all architectures
- **Same model** — Each architecture-model pair uses the same LLM endpoint
- **Same token budget** — 3,500 schema token budget for skip detection; per-model context limits for execution

### Dependent Variables (Metrics)

- **Total Tokens** — Sum of prompt + completion across all LLM calls
- **Prompt Tokens** — Schema overhead + conversation history
- **Completion Tokens** — Model response length
- **Schema Tokens** — Tool schema injection cost (measured via tiktoken cl100k_base)
- **Agent Hops** — Number of sequential LLM invocations
- **Tools Used** — Number of distinct tools actually called
- **Latency** — End-to-end wall-clock time
- **Cost (USD)** — Estimated from model-specific token pricing

### Threats to Validity

1. **LLM non-determinism** — Results vary across runs due to sampling. Mitigation: temperature=0.1, multiple models tested.
2. **Simulated MCPs** — Experiment 1 uses mock servers. Real transport overhead not captured. Mitigation: Experiment 2 validates with real npx/uvx servers.
3. **Single prompt per workflow** — Each workflow has one task definition. Results may differ with different phrasings.
4. **Provider rate limits** — API rate limiting affects latency measurements. Mitigation: rate limiter + retry with backoff.
5. **Tokenizer mismatch** — Schema tokens computed via tiktoken (cl100k_base), not the target model's tokenizer. Relative rankings unaffected.

### Reproducibility

All results include run metadata:
```json
{
  "schema_version": "1.0",
  "run_id": "<uuid>",
...
pip install -e ".[dev]"
# Configure .env with same model/provider
cd experiment1_scalability && python run_all.py
```
